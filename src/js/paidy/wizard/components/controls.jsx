...
const StoreUrlTextControl = ( { value } ) => {
	return (
		<div className="components-base-control">
			...
			<div className="paidy-readonly-field">{ value }</div>
		</div>
	);
};
...
const RepresentativeDateOfBirthTextControl = ( { value, onChange } ) => {
	return (
		<TextControl
			...
			help={ __(
				'Please select a date from the calendar icon on the far right.',
				'woocommerce-for-japan'
			) }
			value={ value }
			type="date"
	...
	return (
		<RadioControl
			...
			options={ [
				...
				{
					label: __( 'Over 50,000 yen', 'woocommerce-for-japan' ),
...
const SecuritySurvey01RadioControl = ( { value, onChange } ) => {
	return (
		<RadioControl
			label={ __(
				'Do you use two-step or two-factor authentication to prevent compromised accounts?',
				'woocommerce-for-japan'
			) }
			selected={ value }
			onChange={ onChange }
...
const SecuritySurvey01TextControl = ( { value, onChange } ) => {
	return (
		<TextareaControl
			label={ __(
				'If the answer is "no", please provide an alternative.',
				'woocommerce-for-japan'
			) }
			value={ value }
			onChange={ onChange }
...
const SecuritySurvey11CheckControl = ( { value, onChange } ) => {
	return (
		<CheckboxControl
			label={ __(
				'Restricting access from suspicious IP addresses',
				'woocommerce-for-japan'
			) }
			checked={ value }
			help={ __(
				'This refers to restricting access from suspicious IP addresses, such as those from overseas, using firewalls, WAFs, apps, etc.',
				'woocommerce-for-japan'
			) }
			onChange={ onChange }
		/>
...
const SecuritySurvey12CheckControl = ( { value, onChange } ) => {
	return (
		<CheckboxControl
			label={ __(
				'Identity verification using two-factor authentication, etc.',
				'woocommerce-for-japan'
			) }
			checked={ value }
			help={ __(
				'This refers to increasing security strength by combining multiple levels or elements of authentication, such as biometric authentication using fingerprints or faces, or email or SMS authentication, in addition to ID and password.',
				'woocommerce-for-japan'
			) }
			onChange={ onChange }
		/>
...
const SecuritySurvey13CheckControl = ( { value, onChange } ) => {
	return (
		<CheckboxControl
			...
			checked={ value }
			help={ __(
				'It is a security technology that constantly monitors communications on the network and detects fraudulent transactions in advance, such as the use of credit cards by third parties or impersonation.',
				'woocommerce-for-japan'
			) }
			onChange={ onChange }
		/>
...
const SecuritySurvey14CheckControl = ( { value, onChange } ) => {
	return (
		<CheckboxControl
			...
			help={ __(
				"This technology tracks online behavior by using the characteristics of the operating environment of the user's device as a kind of fingerprint.",
				'woocommerce-for-japan'
			) }
			checked={ value }
			onChange={ onChange }
...
const SecuritySurvey10TextAreaControl = ( { value, onChange } ) => {
	return (
		<TextareaControl
			label={ __(
				'If there is more than one question with a "yes" answer, please provide an alternative solution.',
				'woocommerce-for-japan'
			) }
			value={ value }
			onChange={ onChange }
...
const SecuritySurvey08RadioControl = ( { value, onChange } ) => {
	return (
		<RadioControl
			label={ __(
				'We have not received any administrative penalties under the Specified Commercial Transactions Law in the past five years.',
				'woocommerce-for-japan'
			) }
			selected={ value }
			onChange={ onChange }
...
const SecuritySurvey09RadioControl = ( { value, onChange } ) => {
	return (
		<RadioControl
			label={ __(
				'I have never been sued in a civil lawsuit or received an unfavorable judgment based on actions that violated the Consumer Contract Act.',
				'woocommerce-for-japan'
			) }
			selected={ value }
			onChange={ onChange }
