...
class WC_Gateway_BANK_JP extends WC_Payment_Gateway {
	...
	/**
	 * The generate_account_details_html function.
	 */
	public function generate_account_details_html() {
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label>
					<?php esc_html_e( 'Account Details', 'woocommerce-for-japan' ); ?>:
				</label>
			</th>
			<td class="forminp" id="bankjp_accounts">
				<table class="widefat wc_input_table sortable" cellspacing="0">
					<thead>
						<tr>
							<th class="sort">&nbsp;</th>
							<th><?php esc_html_e( 'Bank Name', 'woocommerce-for-japan' ); ?></th>
							<th><?php esc_html_e( 'Bank Branch', 'woocommerce-for-japan' ); ?></th>
							<th><?php esc_html_e( 'Bank Type', 'woocommerce-for-japan' ); ?></th>
							<th><?php esc_html_e( 'Account Number', 'woocommerce-for-japan' ); ?></th>
							<th><?php esc_html_e( 'Account Name', 'woocommerce-for-japan' ); ?></th>
						</tr>
					</thead>
					<tfoot>
						<tr>
							<th colspan="7"><a href="#" class="add button"><?php esc_html_e( '+ Add Account', 'woocommerce-for-japan' ); ?></a> <a href="#" class="remove_rows button"><?php esc_html_e( 'Remove selected account(s)', 'woocommerce-for-japan' ); ?></a></th>
						</tr>
					</tfoot>
						...
						if ( $this->account_details ) {
							foreach ( $this->account_details as $account ) {
								...
								echo '<tr class="account">
									<td class="sort"></td>
									<td><input type="text" value="' . esc_attr( $account['bank_name'] ) . '" name="bankjp_bank_name[' . esc_attr( $i ) . ']" /></td>
									<td><input type="text" value="' . esc_attr( $account['bank_branch'] ) . '" name="bankjp_bank_branch[' . esc_attr( $i ) . ']" /></td>
									<td><input type="text" value="' . esc_attr( $account['bank_type'] ) . '" name="bankjp_bank_type[' . esc_attr( $i ) . ']" /></td>
									<td><input type="text" value="' . esc_attr( $account['account_number'] ) . '" name="bankjp_account_number[' . esc_attr( $i ) . ']" /></td>
									<td><input type="text" value="' . esc_attr( $account['account_name'] ) . '" name="bankjp_account_name[' . esc_attr( $i ) . ']" /></td>
								</tr>';
							}
						}
						?>
	...
	public function save_account_details() {
		...
		if ( isset( $_POST['bankjp_account_name'] )
			&& isset( $_POST['bankjp_account_number'] )
			...
			$bank_branches   = wc_clean( wp_unslash( $_POST['bankjp_bank_branch'] ) );// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function thankyou_page( $order_id ) {
		...
		if ( 'yes' === $this->display_order ) {
			echo wp_kses_post( $bank_detail );
	...
	private function html_bank_details( $order_id = '' ) {
		...
		if ( ! empty( $bankjp_accounts ) ) {
			...
			foreach ( $bankjp_accounts as $bankjp_account ) {
				...
				$account_field = apply_filters(
					'woocommerce_bankjp_account_fields',
					array(
						'account_info' => array(
...
if ( get_option( 'wc4jp-bankjp' ) ) {
	add_filter( 'woocommerce_payment_gateways', array( 'WC_Gateway_BANK_JP', 'add_gateway' ) );
}
