...
function peachpay_settings_payment() {
	...
	if ( get_option( 'peachpay_api_access_denied' ) && ! peachpay_is_test_mode() ) {
	...
	add_settings_field(
		...
		__( 'Google Pay / Apple Pay', 'woocommerce-for-japan' ),
	...
	add_settings_field(
		...
		__( 'Enable Stripe', 'woocommerce-for-japan' ),
	...
	add_settings_field(
		...
		__( 'Enable PayPal', 'woocommerce-for-japan' ),
...
function peachpay_section_payment_cannot_continue_html() {
	...
	?>
	<p><?php _e( 'To continue setting up PeachPay, please', 'woocommerce-for-japan' ); ?>&nbsp<a href="<?php echo $retry_url; ?>">&nbsp<?php _e( 'choose "Approve" on the permission screen', 'woocommerce-for-japan' ); ?></a>.</p>
		<?php
}
...
function peachpay_field_stripe_cb() {
	if ( get_option( 'peachpay_connected_stripe_account' ) ) {
		?>
		<p><span class="dashicons dashicons-yes-alt"></span> <?php _e( "You've successfully connected the Stripe account associated with", 'woocommerce-for-japan' ); ?>&nbsp<strong><?php echo get_option( 'peachpay_connected_stripe_account' )['email']; ?></strong>
		<?php } else { ?>
		<a
			href="<?php echo peachpay_generate_stripe_url( get_site_url() ); ?>"
			class="stripe-connect"
		>
			<span><?php _e( 'Connect with', 'woocommerce-for-japan' ); ?></span>
		</a>
		<style>
			.stripe-connect {
				background: #635bff;
...
function peachpay_field_stripe_payment_request_cb() {
	...
	?>
	>
	<label for="peachpay_stripe_payment_request"><?php _e( 'Enable Google Pay and Apple Pay buttons', 'woocommerce-for-japan' ); ?></label>
		<?php
}
...
function peachpay_field_enable_stripe_html() {
	...
	<label for="peachpay_enable_stripe">
		<?php _e( 'Show Stripe in the checkout window', 'woocommerce-for-japan' );?>
	</label>
	<?php
}
...
function peachpay_field_paypal_cb() {
	if ( peachpay_is_test_mode() ) {
		?>
		<p>PayPal is in <a target="_blank" href="https://docs.peachpay.app/paypal#test-mode">test mode</a>. You can use the guest checkout option to pay with PayPal in test mode.</p>
		<?php
	} elseif ( get_option( 'peachpay_paypal_signup' ) ) {
		?>
		<p>
			<span class="dashicons dashicons-yes-alt"></span> <?php _e( "You've successfully connected your PayPal account that has Merchant ID", 'woocommerce-for-japan' ); ?>&nbsp<b><?php echo esc_html( peachpay_get_paypal_merchant_id() ); ?></b>
		</p>
		<br>
		<a class="button button-unlink-paypal" href="?page=peachpay&tab=payment&unlink_paypal" ><?php _e( 'Unlink PayPal', 'woocommerce-for-japan'); ?></a>
		<?php
	} else {
		?>
		<a href="<?php echo esc_url( peachpay_get_paypal_link() ); ?>" title="PayPal">
			...
			<?php _e( 'Connect your PayPal business account to start accepting payments through PayPal', 'woocommerce-for-japan' ); ?>
			</div>
		</a>
			<?php
	}
...
function peachpay_field_paypal_box_cb() {
	...
	<input
		...
		?>
	>
	<label for="peachpay_paypal_box"><?php _e( 'Show PayPal in the checkout window', 'woocommerce-for-japan' );?></label>
	<?php
}
