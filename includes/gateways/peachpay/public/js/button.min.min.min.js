...
<div id="pp-button-container" class="button-container pp-button-container ${peachpay_data.test_mode&&!peachpay_data.wp_admin_or_editor?"hide":""}">
	...
	<div id="payment-methods-container" class="cc-company-logos">
		...
		<img class="${peachpay_data.feature_support.stripe_payment_method.metadata.afterpay_clearpay_payments?"cc-logo":"hide"}" src="${peachpay_data.plugin_asset_url}public/img/marks/afterpay.svg"/>
...
<div id="pp-button-container-mini" class="button-container pp-button-container ${peachpay_data.test_mode&&!peachpay_data.wp_admin_or_editor?"hide":""}">
	<button id="pp-button-mini" class="pp-button ${peachpay_data.disable_default_font_css=="1"?"":"pp-button-default-font"} ${peachpay_pageTypeClass()} ${peachpay_data.button_custom_classes}" type="button" style="display: block;">
	...
	<div id="payment-methods-container-minicart" class="cc-company-logos">
		...
		<img class="${peachpay_data.feature_support.stripe_payment_method.metadata.afterpay_clearpay_payments?"cc-logo-sidebar":"hide"}" src="${peachpay_data.plugin_asset_url}public/img/marks/afterpay.svg"/>
...
<div class="pp-break"></div>
`;function peachpay_checkoutFormHTMLTemplate(){const assetURL=peachpay_data.plugin_asset_url;const version=peachpay_data.version;return`
...
`}const peachpay_stock_observer=new MutationObserver(function(){let needs_muted=false;const checkout_button=document.getElementsByClassName("single_add_to_cart_button");if(checkout_button[0]){const cL=checkout_button[0].classList;if(cL.contains("disabled")){needs_muted=true}}const button=document.querySelector("#pp-button");if(button){if(needs_muted){button.classList.add("pp-button-mute")}else{button.classList.remove("pp-button-mute")}}});document.addEventListener("DOMContentLoaded",()=>peachpay_addCustomMerchantStyles());function peachpay_addCustomMerchantStyles(){if(!peachpay_isElementor()&&peachpay_data.button_effect==="fade"){peachpay_buttonFade()}if(!peachpay_isElementor()&&!peachpay_data.disable_default_font_css){peachpay_defaultFontCSS()}if(document.querySelector("#payment-methods-container")&&!peachpay_data.button_display_payment_method_icons){document.querySelector("#payment-methods-container").classList.add("hide")}peachpay_customButtonCSS();if(document.querySelector("#button-icon-regular")){update_buttonIcon(peachpay_data.button_icon,"regular")}}function peachpay_buttonFade(){const body=document.querySelector("body");if(body){body.insertAdjacentHTML("beforeend",peachpay_buttonFadeCSS)}}function peachpay_customButtonCSS(){const $body=document.querySelector("body");if($body){const $style=document.createElement("style");$style.id="pp-custom-css";$style.appendChild(document.createTextNode(peachpay_data.button_custom_css.trim()));$body.insertAdjacentElement("beforeend",$style)}}function peachpay_defaultFontCSS(){const ppButtons=document.querySelectorAll(".pp-button");if(ppButtons[0]){ppButtons.forEach(ppButton=>{ppButton.classList.add("pp-button-default-font")})}}const peachpay_buttonFadeCSS=`
