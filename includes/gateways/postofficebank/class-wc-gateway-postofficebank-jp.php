...
class WC_Gateway_PostOfficeBank_JP extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->account_details = get_option(
			'woocommerce_postofficebankjp_accounts',
			array(
				array(
	...
	public function generate_account_details_html() {
		...
		?>
		<tr valign="top">
			<th scope="row" class="titledesc"><?php esc_html_e( 'Post Office Account Details', 'woocommerce-for-japan' ); ?>:</th>
			<td class="forminp" id="bankjp_accounts">
				<table class="widefat wc_input_table sortable" cellspacing="0">
					<thead>
						<tr>
							<th class="sort">&nbsp;</th>
							<th><?php esc_html_e( 'Bank Symbol', 'woocommerce-for-japan' ); ?></th>
					...
					<tfoot>
						<tr>
							<th colspan="4"><a href="#" class="add button"><?php esc_html_e( '+ Add Account', 'woocommerce-for-japan' ); ?></a> <a href="#" class="remove_rows button"><?php esc_html_e( 'Remove selected account(s)', 'woocommerce-for-japan' ); ?></a></th>
						</tr>
					</tfoot>
						...
						if ( $this->account_details ) {
							foreach ( $this->account_details as $account ) {
								...
								echo '<tr class="account">
		                			<td class="sort"></td>
		                			<td><input type="text" value="' . esc_attr( $account['bank_symbol'] ) . '" name="postofficebankjp_bank_symbol[' . esc_attr( $i ) . ']" /></td>
		                			<td><input type="text" value="' . esc_attr( $account['account_number'] ) . '" name="postofficebankjp_account_number[' . esc_attr( $i ) . ']" /></td>
		                			<td><input type="text" value="' . esc_attr( $account['account_name'] ) . '" name="postofficebankjp_account_name[' . esc_attr( $i ) . ']" /></td>
			                    </tr>';
							}
						}
						?>
	 ...
	 * @param bool     $plain_text Plain text email or HTML email.
	 * @return void
	 */
		...
		if ( ! empty( $postofficebankjp_accounts ) ) {
			...
			foreach ( $postofficebankjp_accounts as $postofficebankjp_account ) {
				...
				$account_field = apply_filters(
					'woocommerce_postofficebankjp_account_fields',
					array(
