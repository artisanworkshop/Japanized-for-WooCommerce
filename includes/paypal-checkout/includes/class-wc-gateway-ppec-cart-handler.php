...
class WC_Gateway_PPEC_Cart_Handler {
	...
	public function wc_ajax_generate_cart() {
		...
		if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], '_wc_ppec_generate_cart_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			wp_die( __( 'Cheatin&#8217; huh?', 'woocommerce-for-japan' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	...
	protected function set_customer_data( $data ) {
		...
		//Add code for Japanese user for PayPal 2020/08/21
        $wc_countries = new WC_Countries;
        $wc_states = $wc_countries->get_states();
        $jp_states = $wc_states['JP'];
        if(isset($jp_states[$billing_state])){
            $shipping_state = $jp_states[$billing_state];
        }
        if(isset($jp_states[$shipping_state])){
            $shipping_state = $jp_states[$shipping_state];
        }
	...
	public function display_paypal_button_product() {
		...
		<div class="wcppec-checkout-buttons woo_pp_cart_buttons_div">
			...
			<a href="<?php echo esc_url( add_query_arg( array( 'startcheckout' => 'true' ), wc_get_page_permalink( 'cart' ) ) ); ?>" id="woo_pp_ec_button_product" class="wcppec-checkout-buttons__button">
				<img src="<?php echo esc_url( $express_checkout_img_url ); ?>" alt="<?php esc_attr_e( 'Check out with PayPal', 'woocommerce-for-japan' ); ?>" style="width: auto; height: auto;">
			</a>
			<?php endif; ?>
		</div>
		<?php
	...
	public function display_paypal_button() {
		...
		<div class="wcppec-checkout-buttons woo_pp_cart_buttons_div">
			...
			<?php if ( has_action( 'woocommerce_proceed_to_checkout', 'woocommerce_button_proceed_to_checkout' ) ) : ?>
				<div class="wcppec-checkout-buttons__separator">&mdash; <?php esc_html_e( 'OR', 'woocommerce-for-japan' ); ?> &mdash;</div>
			<?php endif; ?>
			...
			<a href="<?php echo esc_url( add_query_arg( array( 'startcheckout' => 'true' ), wc_get_page_permalink( 'cart' ) ) ); ?>" id="woo_pp_ec_button" class="wcppec-checkout-buttons__button">
				...
				<img src="<?php echo esc_url( $paypal_credit_img_url ); ?>" alt="<?php esc_attr_e( 'Pay with PayPal Credit', 'woocommerce-for-japan' ); ?>" style="width: auto; height: auto;">
				</a>
				<?php endif; ?>
	...
	public function display_mini_paypal_button() {
		...
		<a href="<?php echo esc_url( add_query_arg( array( 'startcheckout' => 'true' ), wc_get_page_permalink( 'cart' ) ) ); ?>" id="woo_pp_ec_button" class="wcppec-cart-widget-button">
			<img src="<?php echo esc_url( 'https://www.paypalobjects.com/webstatic/en_US/i/btn/png/gold-rect-paypalcheckout-26px.png' ); ?>" alt="<?php esc_attr_e( 'Check out with PayPal', 'woocommerce-for-japan' ); ?>" style="width: auto; height: auto;">
		</a>
		<?php endif; ?>
		<?php
	}
	...
	public function enqueue_scripts() {
		...
		if ( 'yes' !== $settings->use_spb && $is_cart ) {
			...
			$data                    = array(
				...
				'cancel_url'           => '',
				'generic_error_msg'    => wp_kses( __( 'An error occurred while processing your PayPal payment. Please contact the store owner for assistance.', 'woocommerce-for-japan' ), array() ),
			);
