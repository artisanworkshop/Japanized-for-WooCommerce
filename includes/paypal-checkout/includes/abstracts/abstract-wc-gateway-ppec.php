...
abstract class WC_Gateway_PPEC extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'PayPal Checkout', 'woocommerce-for-japan' );
		$this->method_description = __( 'Allow customers to conveniently checkout directly with PayPal.', 'woocommerce-for-japan' );
		...
		if ( empty( $_GET['woo-paypal-return'] ) && 'yes' !== $this->get_option( 'use_spb', 'yes' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$this->order_button_text = __( 'Continue to payment', 'woocommerce-for-japan' );
		}
	...
	public function process_payment( $order_id ) {
		...
		if ( $checkout->is_started_from_checkout_page() ) {
			...
			try {
				...
				wc_add_notice( __( 'Sorry, an error occurred while trying to process your payment. Please try again.', 'woocommerce-for-japan' ), 'error' );
			} catch ( PayPal_API_Exception $e ) {
	...
	private function get_certificate_info( $cert_string ) {
		if ( ! strlen( $cert_string ) ) {
			return __( 'No API certificate on file.', 'woocommerce-for-japan' );
		}
		...
		if ( $cert_info ) {
			...
			$expires = __( 'expires on %1$s (%2$s)', 'woocommerce-for-japan' );

			if ( $valid_until < time() ) {
				// Display in red if the cert is already expired
				// Translators: placeholders are a date in local format and its timezone.
				$expires = '<span style="color: red;">' . __( 'expired on %1$s (%2$s)', 'woocommerce-for-japan' ) . '</span>';
			...
			$output = __( 'The certificate on file is not valid.', 'woocommerce-for-japan' );
		}
	...
	protected function validate_active_credentials() {
		...
		if ( ! $has_username ) {
			$errors[] = __( 'Error: You must enter API username.', 'woocommerce-for-japan' );
		}

		if ( ! $has_password ) {
			$errors[] = __( 'Error: You must enter API password.', 'woocommerce-for-japan' );
		}

		if ( ! $has_credential ) {
			$errors[] = __( 'Error: You must provide API signature or certificate.', 'woocommerce-for-japan' );
		}
		...
		if ( $has_username && $has_password && $has_credential ) {
			if ( is_a( $creds, 'WC_Gateway_PPEC_Client_Credential_Signature' ) ) {
				try {
					...
					if ( ! $payer_id ) {
						$errors[] = __( 'Error: The API credentials you provided are not valid.  Please double-check that you entered them correctly and try again.', 'woocommerce-for-japan' );
					}
				} catch ( PayPal_API_Exception $ex ) {

					$errors[] = __( 'An error occurred while trying to validate your API credentials. Unable to verify that your API credentials are correct.', 'woocommerce-for-japan' );
				}
				...
				if ( false === $cert ) {
					$errors[] = __( 'Error: The API certificate is not valid.', 'woocommerce-for-japan' );
				}
				...
				if ( $valid_until < time() ) {
					$errors[] = __( 'Error: The API certificate has expired.', 'woocommerce-for-japan' );
				} elseif ( $cert_info['subject']['CN'] !== $creds->get_username() ) {
					$errors[] = __( 'Error: The API username does not match the name in the API certificate. Make sure that you have the correct API certificate.', 'woocommerce-for-japan' );
				}
